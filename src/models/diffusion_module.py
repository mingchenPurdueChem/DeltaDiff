    ...
    def predict_step(
        ...
        with torch.enable_grad():   
            ...
            def forward_backward(rigids_0: Rigid, t_delta: float):
                ...
                ## new guidance ###
                residue_index_rep = batch["residue_index"].repeat(batch_size, 1)
                chain_index_rep = batch["chain_index"].repeat(batch_size, 1)

                wt_aatype = _feats["aatype"]
                mut_aatype = None

                trans_guidance = getattr(self.diffuser.trans_diffuser, "guidance", None)
                if trans_guidance is not None:
                    mut_aatype = trans_guidance.build_mutant_aatype(
                        wt_aatype=wt_aatype,
                        residue_index=residue_index_rep,
                        chain_index=chain_index_rep,
                    )
                if mut_aatype is not None:
                    extra['aatype'] = mut_aatype[0].detach().cpu().numpy()
                ## DEBUG     
                if trans_guidance is not None and not hasattr(trans_guidance, "_debug_mutation_printed"):
                    spec = trans_guidance.mutations[0]

                    if "resid" in spec:
                        target_mask = (residue_index_rep[0] == int(spec["resid"]))
                        if "chain_index" in spec:
                            target_mask = target_mask & (chain_index_rep[0] == int(spec["chain_index"]))
                    else:
                        target_mask = torch.zeros_like(wt_aatype[0], dtype=torch.bool)
                        target_mask[int(spec["seq_idx"])] = True

                    print("\n[TORCHMD DEBUG] mutation spec:", spec)
                    print("[TORCHMD DEBUG] matched residue_index:", residue_index_rep[0][target_mask].detach().cpu().tolist())
                    print("[TORCHMD DEBUG] matched chain_index:", chain_index_rep[0][target_mask].detach().cpu().tolist())
                    print("[TORCHMD DEBUG] wt_aatype at target:", wt_aatype[0][target_mask].detach().cpu().tolist())
                    print("[TORCHMD DEBUG] mut_aatype at target:", mut_aatype[0][target_mask].detach().cpu().tolist())

                    trans_guidance._debug_mutation_printed = True
                ## new guidance ###
                ...
                if use_grad:
                    ...
                    for t in ts:
                        ...
                        if temp_set_grad_flag:
                            with torch.enable_grad():
                                _feats['rigids_t'] = _feats['rigids_t'].clone().detach().requires_grad_(True)
                        ...
                        if t == min_t:
                            ...
                            rigids_pred = self.diffuser.reverse(
                                ...
                                residue_mask=_feats["residue_mask"],
                            )
                            if temp_set_grad_flag:
                                temp_set_grad_flag = False
                            _feats['rigids_t'] = rigids_pred.to_tensor_7()

                    aatype_out = mut_aatype if mut_aatype is not None else _feats['aatype']
                    atom37 = compute_backbone(rigids_pred, out['psi'], aatype=aatype_out)[0]
